...
def ID_4(
    ...
    with tf.variable_scope(scope):
        ...
        #net = tf.nn.lrn(net, depth_radius=5, bias=1.0, alpha=0.0001, beta=0.75,name='norm1')
        with tf.variable_scope("layer0-0"):
            ...
            net = batch_norm(net, is_training ,scope = "layer00_bn")
            net = tf.nn.relu(features = net, name = "ReLU")	
            #net, indices, unpool_shape = max_pooling(net, kernel_size = 2, strides = 2, padding = "SAME")                
            #net = tf.nn.relu(features = net, name = "ReLU")	
        ...
        with tf.variable_scope("layer0-1"):
            ...
            net = batch_norm(net, is_training ,scope = "layer01_bn")
            net = tf.nn.relu(features = net, name = "ReLU")	
            ...
            #net = tf.nn.relu(features = net, name = "ReLU")	
            net = tf.cond(is_training, lambda: tf.layers.dropout(net, 0.5), lambda: net)
        with tf.variable_scope("layer0-2"):
            net = dilated_conv2D( net,
                          kernel_size    = 3,
                          rate       = 2, 
                          output_channel = 64,
                          ...
                          padding        = "SAME", 
                          scope          = "conv3x3")    
            net = batch_norm(net, is_training ,scope = "layer02_bn")      
            net = tf.nn.relu(features = net, name = "ReLU")	 
           # net, indices2, unpool_shape2 = max_pooling(net, kernel_size = 2, strides = 2, padding = "SAME")            
            #net = tf.nn.relu(features = net, name = "ReLU")	     
            #net = tf.cond(is_training, lambda: tf.layers.dropout(net, 0.5), lambda: net)     
        ...
        with tf.variable_scope("layer12"):
            net = dilated_conv2D( net,
                          ...
                          padding        = "SAME", 
                          scope          = "conv3x3")
            net = batch_norm(net, is_training ,scope = "layer12_bn")
        ...
        with tf.variable_scope("layer13"):
            net = dilated_conv2D( net,
                          ...
                          padding        = "SAME", 
                          scope          = "conv3x3")
            net = batch_norm(net, is_training ,scope = "layer13_bn")
        ...
        with tf.variable_scope("layer2"):
            ...
            net = tf.nn.relu(features = net, name = "ReLU")	 
            #net, indices5, unpool_shape5 = max_pooling(net, kernel_size = 2, strides = 2, padding = "SAME")     
        ...
        with tf.variable_scope("layer10"):
            ...
            net = conv2D( net,
                          ...
                          padding        = "SAME", 
                          scope          = "conv3x3")
            net = batch_norm(net,is_training , scope = "layer10_bn")           
            net = tf.nn.relu(features = net, name = "ReLU")      
